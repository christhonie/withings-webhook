...
// 🔧 NUOVA FUNZIONE: Ottieni access token valido (con cache intelligente)
async function getValidAccessToken(userid, env) {
  const tokenKey = `token_data_${userid}`;
  
  try {
    // Controlla se hai già un token valido in cache
    const tokenDataStr = await env.MY_KV.get(tokenKey);
    if (tokenDataStr) {
      const tokenData = JSON.parse(tokenDataStr);
      const now = Math.floor(Date.now() / 1000);
      
      // Se token valido con buffer di 60 secondi, usalo
      if (tokenData.expires_at && now < tokenData.expires_at - 60) {
        console.log(`✅ Token valido per userid ${userid}, scade tra ${tokenData.expires_at - now}s`);
        return tokenData.access_token;
      }
      console.log(`⏰ Token scaduto per userid ${userid}, faccio refresh`);
    }
    
    // Token scaduto o mancante, fai refresh
    return await refreshAccessToken(userid, env);
    
  } catch (error) {
    console.error(`❌ Errore getValidAccessToken per userid ${userid}:`, error);
    throw error;
  }
}

// 🔧 NUOVA FUNZIONE: Refresh token con retry automatico
async function refreshAccessToken(userid, env, retryCount = 0) {
  ...
  try {
    ...
    if (!refreshToken) {
      throw new Error(`Nessun refresh token per userid: ${userid}`);
    }

    console.log(`🔄 Refresh token per userid ${userid} (tentativo ${retryCount + 1})`);
    ...
    // 🚨 GESTIONE ERRORE 601 (troppo veloce)
    if (tokenJson.status === 601 && retryCount < 3) {
      const waitSeconds = tokenJson.body?.wait_seconds || 10;
      console.log(`⏳ Errore 601 per userid ${userid}, aspetto ${waitSeconds}s prima del retry...`);
      
      // Aspetta e riprova
      await new Promise(resolve => setTimeout(resolve, waitSeconds * 1000));
      return await refreshAccessToken(userid, env, retryCount + 1);
    }
    
    if (tokenJson.status !== 0 || !tokenJson.body?.access_token) {
      throw new Error(`Errore refresh token: ${JSON.stringify(tokenJson)}`);
    }

    // 💾 Salva token data completo
    const tokenData = {
    ...
    // Aggiorna entrambe le chiavi
    await env.MY_KV.put(tokenKey, JSON.stringify(tokenData));
    await env.MY_KV.put(refreshKey, tokenJson.body.refresh_token);
    
    console.log(`✅ Token refreshed per userid ${userid}, scade tra ${tokenJson.body.expires_in}s`);
    return tokenJson.body.access_token;
    
  } catch (error) {
    console.error(`❌ Errore refreshAccessToken per userid ${userid}:`, error);
    throw error;
  }
}
...
async function handleNotify(payload, env) {
  try {
    ...
    // 🔧 MODIFICA PRINCIPALE: Usa la nuova funzione per ottenere token valido
